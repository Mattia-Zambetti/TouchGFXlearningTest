...
osThreadId ADC_PotTaskHandle;
osThreadId LedBlinkHandle;
osSemaphoreId binaryADC_NewValuesSemHandle;
/* USER CODE BEGIN PV */
...
void StartADC_PotTask(void const * argument);
...
int main(void)
{
  ...
  /* definition and creation of binaryADC_NewValuesSem */
  osSemaphoreDef(binaryADC_NewValuesSem);
  binaryADC_NewValuesSemHandle = osSemaphoreCreate(osSemaphore(binaryADC_NewValuesSem), 1);
  ...
  /* definition and creation of ADC_PotTask */
  osThreadDef(ADC_PotTask, StartADC_PotTask, osPriorityNormal, 0, 128);
  ADC_PotTaskHandle = osThreadCreate(osThread(ADC_PotTask), NULL);
...
/* USER CODE BEGIN Header_StartADC_PotTask */
/**
* @brief Function implementing the ADC_PotTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartADC_PotTask */
void StartADC_PotTask(void const * argument)
{
  /* USER CODE BEGIN StartADC_PotTask */
	PollingAnalogInit ();
  /* Infinite loop */
  for(;;)
  {
		PollingAnalogRoutine();
    osDelay(1);
  }
  /* USER CODE END StartADC_PotTask */
}
